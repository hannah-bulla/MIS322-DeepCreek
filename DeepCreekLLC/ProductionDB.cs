...
namespace DeepCreekLLC.Database
{
    public class ProductionBatch
    {
        ...
        public string ModelCode { get; set; } = "";   // joined from RodModel
    ...
    {
        public int RodModelID { get; set; }
        public string DisplayText { get; set; } = "";  // e.g. "BR0906E — Better 9'6" Extra Fast"
    }
    ...
    public static class ProductionRepository
    {
        ...
        public static List<ProductionBatch> GetAllBatches()
        {
            ...
            const string sql = @"
                SELECT pb.BatchID, pb.BatchCode, pb.BatchDate,
                       pb.ProductionLineNumber, pb.ShiftNumber,
                       rm.ModelCode, pb.RodModelID,
                       pb.PlannedQty, pb.ActualQty, pb.GoodUnits, pb.DefectUnits
        ...
        public static void InsertBatch(ProductionBatch b)
        {
            const string sql = @"
                INSERT INTO ProductionBatch
                    (BatchCode, BatchDate, ProductionLineNumber, ShiftNumber,
                     RodModelID, PlannedQty, ActualQty, GoodUnits, DefectUnits)
                VALUES
                    (@BatchCode, @BatchDate, @Line, @Shift,
                     @RodModelID, @Planned, @Actual, @Good, @Defect)";

            using var conn = DbConfig.GetConnection();
            using var cmd = new SqlCommand(sql, conn);

            // Always use parameters — never string-concatenate SQL (SQL injection)
        ...
        {
            const string sql = "DELETE FROM ProductionBatch WHERE BatchID = @BatchID";
            using var conn = DbConfig.GetConnection();
            using var cmd = new SqlCommand(sql, conn);
