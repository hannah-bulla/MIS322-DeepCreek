DROP DATABASE IF EXISTS DeepCreekDB;
...
-- DefectCode (codes from QA sheet)
...
-- ProductionBatch (10 rows, format: YYMMDD-Shift.Run, lines 1-2)
...
-- QAInspection (one header per batch; date may differ from batch date)
...
-- QAInspectionDetail (10 rows drawn from the sample QA sheet and defect codes)
...
-- CustomerReturn (10 rows, mix of models and resolutions)
